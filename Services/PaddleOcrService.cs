...
public sealed class PaddleOcrService(AppSettings settings) : ITextCaptureService
{
    ...
    public async Task<IReadOnlyList<OcrTextLine>> CaptureAsync(TargetWindowInfo? target, Rect? region, CancellationToken cancellationToken)
    {
        ...
        var tempImage = ScreenCaptureService.CaptureRegionToPng(captureBounds, Path.GetTempPath(), "not_paddle_ocr");
        try
        {
    ...
    {
        if (!Uri.TryCreate(settings.PaddleOcrEndpoint, UriKind.Absolute, out var endpoint))
        {
            return [new OcrTextLine($"Invalid PaddleOCR endpoint: {settings.PaddleOcrEndpoint}", offset, 1)];
        }
        ...
        try
        {
            ...
            {
                return [new OcrTextLine($"PaddleOCR failed ({(int)response.StatusCode}): {responseText}", offset, 1)];
            }
        ...
        {
            return [new OcrTextLine($"PaddleOCR unavailable: {ex.Message}", offset, 1)];
        }
    }
    ...
    {
        bounds = Rect.Empty;
        foreach (var name in new[] { "bounds", "rect", "bbox" })
        {
            if (item.ValueKind == JsonValueKind.Object &&
                item.TryGetProperty(name, out var value) &&
        ...
        if (item.ValueKind == JsonValueKind.Object &&
            TryReadNumber(item, "x", out var x) &&
            TryReadNumber(item, "y", out var y) &&
            TryReadNumber(item, "width", out var width) &&
            TryReadNumber(item, "height", out var height))
        {
            bounds = new Rect(x, y, width, height);
            return true;
        }

        foreach (var name in new[] { "box", "points", "polygon" })
        {
            if (item.ValueKind == JsonValueKind.Object &&
                item.TryGetProperty(name, out var value) &&
    ...
    private static bool TryReadRect(JsonElement value, out Rect bounds)
    {
        ...
        if (value.ValueKind == JsonValueKind.Object &&
            TryReadNumber(value, "x", out var x) &&
            TryReadNumber(value, "y", out var y) &&
            TryReadNumber(value, "width", out var width) &&
            TryReadNumber(value, "height", out var height))
        {
            bounds = new Rect(x, y, width, height);
            return true;
        }
    ...
    private static bool TryReadPolygon(JsonElement value, out Rect bounds)
    {
        ...
        foreach (var point in value.EnumerateArray())
        {
            ...
            {
                var coords = point.EnumerateArray().Where(c => c.ValueKind == JsonValueKind.Number).Select(c => c.GetDouble()).ToList();
                if (coords.Count >= 2)
                {
                    points.Add(new System.Windows.Point(coords[0], coords[1]));
                }
            }
        }
    ...
    private static string NormalizeLanguage(string sourceLanguage)
    {
        ...
        {
            "ja" or "jpn" or "japanese" => "japan",
            "zh" or "chi_sim" or "chinese" => "ch",
    ...
    private sealed record PaddleOcrRequest(string Image_Base64, string Language, string FileName);
}
